...
### 7. 实体类（Entity）字段可空与默认值约定

**总体原则**

- 是否允许 null 由**业务语义**决定：必需字段不可为 null；可选字段允许为 null，避免用 `0`/`""` 伪造语义。
- 默认值只在**有明确业务含义**时设置（例如数量默认 `0`、状态默认“初始化”）；不强行对所有字段赋默认值。
- 可以合理依赖数据库默认值（如创建时间、版本号等），但**应用层需与之保持一致**并避免双重赋值冲突。

**按类型建议**

| 类型 | 是否允许 null | 说明 |
|------|----------------|------|
| 数字类型（Integer、Long、BigDecimal 等） | 视业务语义 | 无“必有值”语义则可为 null；避免用 `0` 表示“未知/未填” |
| 字符串（String） | 视业务语义 | 避免用 `""` 代替“未填写”；确有空字符串语义时可用 |
| JSON 类型（如扩展字段、JSON 列） | 是 | 可为 null；如需默认，建议空对象/空数组并明确含义 |
| 日期类型（LocalDateTime、Date 等） | 视业务语义 | 创建时间等必填；更新时间/删除时间等“可选时点”允许为 null |

**实施要求**

- 实体类定义：优先使用包装类型表达“可空”；仅在明确不可空且需要默认值时使用基本类型或初始化默认值。
- 保存/更新时：仅对**有明确默认语义**的字段赋值；其余字段保持 null 并让校验规则约束必填性。
- 日期类：必有时点的字段由应用层或数据库统一赋值策略；可选时点字段允许 null。
