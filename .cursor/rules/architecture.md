...
### 7. 实体类（Entity）字段非空与默认值约定

**总体原则**

- 实体类字段除下文明确允许为 null 的类型外，**均不允许为 null**，避免 NPE 与歧义。
- **所有字段在入库/落库前必须设置默认值**；前端未传的字段按类型使用约定默认值，不得依赖数据库默认或留空。

**按类型约定**

| 类型 | 是否允许 null | 默认值 |
|------|----------------|--------|
| 数字类型（Integer、Long、BigDecimal 等） | 否 | `0`（或 `BigDecimal.ZERO` 等对应零值） |
| 字符串（String） | 否 | `""`（空字符串） |
| JSON 类型（如扩展字段、JSON 列） | 是 | 可为 null，不强制默认 |
| 日期类型（LocalDateTime、Date 等） | 视业务合理 | 创建时间等必有时点：必填；**更新时间**：未更新时为 null；**删除时间**：未删除时为 null |

**实施要求**

- 实体类定义：数字、字符串字段在 Java 实体中建议使用包装类型时配合数据库 NOT NULL + 应用层赋默认值，或使用基本类型/明确初始值，确保读写时不为 null。
- 保存/更新时：**所有字段都必须显式赋予默认值后再持久化**。例如数量类字段，前端未传则应在 DTO 转换或 Service 层赋值为 `0`，不得以“未传即 null”写入数据库。
- 日期类：仅在与“可选时点”语义一致时允许 null（如“未更新”“未删除”），其余业务日期应非空并设合理默认（如创建时间取当前时间）。
